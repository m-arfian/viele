...
class ItemController extends Controller {
    ...
    public function actionView($id) {
        ...
        $harga = new Harga('baru');
        $harga->KODE_ITEM = $id;
        
        $listharga = new Harga('search');
        $listharga->unsetAttributes();

        if (isset($_POST['Harga'])) {
            $harga->attributes = $_POST['Harga'];
            if ($harga->validate()) {
                if ($harga->save(false))
                    Yii::app()->user->setFlash('info', Alert::alertSuccess('Harga berhasil dimasukkan'));
                else
                    Yii::app()->user->setFlash('info', Alert::alertDanger('Terjadi error'));
                
                $this->redirect(array('view', 'id' => $model->KODE_ITEM));
            }
        }
